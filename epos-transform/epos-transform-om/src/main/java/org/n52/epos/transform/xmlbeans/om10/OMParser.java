 ...
 */
package org.n52.epos.transform.xmlbeans.om10;
 ...
 *
 */
public class OMParser extends AbstractOMTransformer {
	 ...
	 * @return a {@link List} of parsed {@link MapEposEvent}
	 * @throws XMLHandlingException 
	 * @throws Exception exception
	 */
	public MapEposEvent parseOM(ObservationDocument oDoc) throws TransformationException, XMLHandlingException {

		//the resulting new events
		List<MapEposEvent> events = new ArrayList<MapEposEvent>();
		...
		if (observation == null) throw new TransformationException("No Observation found!");
		 ...
		 */
		if (events.size() > 0) {
			Object tempTime = events.get(0).get(MapEposEvent.START_KEY);
			if (tempTime != null) {
				if ((Long) tempTime != 0) {
					hasTimestamp = true;
				}
		...
		if (!hasTimestamp) {
			...
			if (to instanceof TimeInstantType) {
				...
				for (MapEposEvent me : events) {
					me.put(MapEposEvent.START_KEY, timeStamp);
					me.put(MapEposEvent.END_KEY, timeStamp);
				}
			}
				...
				//put time
				for (MapEposEvent me : events) {
					me.put(MapEposEvent.START_KEY, begin);
					me.put(MapEposEvent.END_KEY, end);
				}
			}
		}
...
//			}
		...
		if (observedProperty.isSetHref()) {
			...
			for (MapEposEvent me : events) {
				me.put(MapEposEvent.OBSERVED_PROPERTY_KEY, obProp);
			}
		} else {
			/*
			...
			if (observedProperty.getPhenomenon() != null && 
					observedProperty.getPhenomenon() instanceof CompositePhenomenonType) {
		...
		XmlObject feature;
		try {
			feature = XmlObject.Factory.parse(foi.toString());
	...
	private void parseFeature(XmlObject feature, List<MapEposEvent> events) throws XmlException, com.vividsolutions.jts.io.ParseException {
		Geometry geom = null;
		String foiID = "";
		...
		if (geom != null) {
			for (MapEposEvent me : events) {
				me.put(MapEposEvent.GEOMETRY_KEY, geom);
			}
		}
		...
		if (!foiID.equals("")) {
			for (MapEposEvent me : events) {
				me.put(MapEposEvent.FOI_ID_KEY, foiID);
			}
		}		
	}


	private boolean parseDataRecord(XmlObject xobj,	List<MapEposEvent> events) {
		...
		if (dr != null) {
			 ...
			 */
			MapEposEvent newEvent = new MapEposEvent(0, 0);
			...
			for (DataComponentPropertyType field : fields) {
				if (field.isSetTime()) {
					...
					newEvent.put(MapEposEvent.START_KEY, dateTime.getMillis());
					newEvent.put(MapEposEvent.END_KEY, dateTime.getMillis());
				}
					...
					if (quan.isSetUom()) {
						if (quan.getUom().isSetCode()) {
							NumberWithUOM temp = UCUMTools.convert(quan.getUom().getCode(), value);
	...
	private void parseDataArray(DataArrayDocument dad, List<Map<String, Object>> pums,
			List<MapEposEvent> events) throws TransformationException {
		...
		while (line.hasMoreTokens()) {
			...
			while (fields.hasMoreTokens()) {
				...
				if ((Boolean) pum.get(IS_UCUM_BOOLEAN_KEY)) {
					...
					newEvent.put(MapEposEvent.START_KEY, timeStamp);
					newEvent.put(MapEposEvent.END_KEY, timeStamp);
				}
				else {
			...
			//set the value field
			if (finalValue != null) {
				newEvent.put(MapEposEvent.VALUE_KEY, finalValue);
			}
	 ...
	 */
	private boolean parseAnyScalarTypes(XmlObject omResultContent, List<MapEposEvent> events) {
		...
		if (anyScalars.length > 0) {
			...
			for (XmlObject as : anyScalars) {
				...
				if (doubleValue != Double.NaN) {
					newEvent.put(MapEposEvent.VALUE_KEY, doubleValue);
					newEvent.put(MapEposEvent.RESULT_KEY, doubleValue);
					...
					newEvent.put(MapEposEvent.STRING_VALUE_KEY,  stringValue);
					newEvent.put(MapEposEvent.RESULT_KEY, stringValue);
					//add to new event with definition (observed property)
					if (definition != null) {
						newEvent.put(definition.replaceAll(":", "__").replaceAll("\\.", "_"),
								stringValue);
	 ...
	 * @param events resulting events are added to this list.
	 */
	private void parseAsSingleValue(XmlAnyTypeImpl anyTypeDoc, List<MapEposEvent> events) {
		double value = Double.NaN;
		...
		if (isDouble) {
			...
			newEvent.put(MapEposEvent.VALUE_KEY, value);
			newEvent.put(MapEposEvent.RESULT_KEY, value);
			newEvent.put(MapEposEvent.DOUBLE_VALUE_KEY, value);
			newEvent.put(MapEposEvent.STRING_VALUE_KEY, "" + value);
			...
			newEvent.put(MapEposEvent.VALUE_KEY, textValue);
			newEvent.put(MapEposEvent.RESULT_KEY, textValue);
			newEvent.put(MapEposEvent.STRING_VALUE_KEY, textValue);
	...
	private Map<String, Object> getPhenomenonAndUomFromComponent(
			DataComponentPropertyType component) throws TransformationException {
		...
		if (component.isSetBoolean()) {
			...
			throw new TransformationException("the " +
	 ...
	 */
	private long getTimestamp(String time, Map<String, Object> pum) {
